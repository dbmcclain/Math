...
#+:WIN32
(fli:define-foreign-function (nextafterf "_nextafter" :source)
    ((x :float)
     (y :float))
  :result-type :float
  :language :c)

(fli:register-module #-:WIN32 "/usr/lib/libm.dylib")

(defparameter eps (- (nextafterf 1.0F0 2.0F0) 1.0F0))
...
(defun t1 ()
  ...
  (pnl "Machine: iMac 1.8 GHz G5"))

(defun t2 ()
  (pnl (if (zerop (v<*> `(,eps 9.0F0 9.0F0)
                        `(,eps 9.0F0 -9.0F0)))
           "Scalar product is summed from left to right"
         "Scalar product is summed from right to left")))

(defun t3 ()
  (let* ((mxmuleps (let* ((z (/ 4.0F0 3.0F0))
                          (u (+ (* 3.0F0 (- 1.0F0 z)) 1.0F0)))
...
(defun t4 ()
  (let* ((x (loop for ix from 1 to 128 collect
         ...
         (n (loop for ix from 1 to 128 collect
                  (+ 65500.0F0 (* 2.0F0 (float ix 1.0F0)))))
